            ...
            <div class="bg-white shadow-md rounded p-6 mb-2">
                ...
                <form action="{{ ($weeklyGoal->weeklyCheck && $weeklyGoal->weeklyAction) ? route('pdca.update-check-action', [$weeklyGoal->weeklyCheck->id, $weeklyGoal->weeklyAction->id]) : route('pdca.create-check-action') }}" method="POST">
                    ...
                    <div class="mb-2">
                        ...
                        <div class="flex mb-1">
                            <input type="hidden" id="rating" name="check-rating" value="{{ old('check-rating', $weeklyGoal->weeklyCheck->review ?? '') }}">
                            @for($i = 0; $i < 5; $i++)
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5 check-star cursor-pointer" data-index="{{ $i }}">
                    ...
                    <div class="mb-2">
                        ...
                        @error('action-description')
                            <p class="text-red-500 text-sm">{{ $message }}</p>
                        @enderror
                        <textarea id="action1" class="placeholder:text-sm placeholder:text-gray-300 shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="action-description" rows="3" placeholder="改善点を入力">{{ old('action-description', $weeklyGoal->weeklyAction->description ?? '') }}</textarea>
                    </div>
